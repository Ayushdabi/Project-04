...
public class ProductModel {
	...
	public long add(ProductBean bean) throws Exception {
		...
		pstmt.setString(4, bean.getProductStatus());
	...
	public void update(ProductBean bean) throws Exception {
		...
		PreparedStatement pstmt = conn.prepareStatement(
				"update st_product set productName = ?, productAmmount = ?,productStatus = ?, purchaseDate = ? where id = ?");

		pstmt.setString(1, bean.getProductName());
		pstmt.setString(2, bean.getProductAmmount());
		pstmt.setString(3, bean.getProductStatus());
	...
	public ProductBean findByPK(long pk) throws Exception {
		...
		while (rs.next()) {
			...
			bean.setProductStatus(rs.getString(4));
	...
	public List search(ProductBean bean, int pageNo, int pageSize) throws Exception {
		...
		if (bean != null) {
			...
			if (bean.getProductStatus() != null && bean.getProductStatus().length() > 0) {
				sql.append(" AND ProductStatus like '" + bean.getProductStatus() + "%'");
			}
